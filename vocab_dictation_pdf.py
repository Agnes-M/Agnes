#!/usr/bin/env python3
"""Remove English words from vocabulary PDFs for Chinese-to-English dictation."""
...
# English tokens: words, abbreviations (adj./n./v.), and parenthetical forms like humo(u)r.
...
def find_english_redaction_rects(page: fitz.Page) -> list[fitz.Rect]:
    rects: list[fitz.Rect] = []
    for chars in iter_line_chars(page):
        line_text, index_mapping = normalize_line(chars)
        for match in ENGLISH_TOKEN_PATTERN.finditer(line_text):
...
def redact_vocab_pdf(
    input_pdf: Path, output_pdf: Path, fill_color: tuple[float, float, float]
) -> int:
    ...
    try:
        for page in doc:
            page_rects = find_english_redaction_rects(page)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="遮盖词汇 PDF 中的英文单词，保留中文释义，便于根据中文默写英文。"
    )
...
def main() -> int:
    ...
    for source_pdf in pdf_files:
        ...
        redaction_count = redact_vocab_pdf(source_pdf, target_pdf, fill_color)
        total_files += 1
        total_redactions += redaction_count
        print(f"[OK] {source_pdf} -> {target_pdf} (遮盖英文 {redaction_count} 处)")

    print(f"完成，共处理 {total_files} 个 PDF，遮盖英文 {total_redactions} 处。")
    return 0
