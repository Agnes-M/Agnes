...
项目支持两种模式，在 `app.js` 中切换：

| 模式 | 配置 | 适用场景 |
|------|------|----------|
| **本地存储**（默认） | `useLocalStorage: true` | 无需云开发，单机试用、演示 |
| **云开发** | `useLocalStorage: false` + 填写 `cloudEnvId` | 多人协作、数据云端同步 |

本地模式首次启动会自动加载 `seed-data/` 中的示例数据，数据保存在手机/模拟器本地，**换设备或清缓存会丢失**。
